...
- Прошивка ~1.19 МБ (Wi-Fi + HTTP + MQTT + TLS/OTA; кодек обрезан до
  MP3-only через Kconfig `esp_audio_codec` — было 1.75 МБ, освобождено
  ~560 КБ); слот 1.94 МБ, запас ~41%.
...
7. ~~Галерея звуков~~ — готово (2026-09-16): `GET /api/audio` (список),
   `POST /api/audio` (загрузка без воспроизведения), `POST
   /api/audio/{name}/play` (тело `{"device","loop"}`), `PATCH`/
   `DELETE /api/audio/{name}`; метаданные (title/size/uploadedAt) в
   `tracks.json` рядом с mp3 в `/data`. Во фронте — секция «Галерея» с
   селектором устройства, play/переименование/удаление.
   Отложено: коллекции (плейлисты) — группировка треков, порядок,
   проигрывание подряд.
8. ~~Зацикливание~~ — готово (2026-09-16): MQTT-команда `loop <url>` —
   прошивка повторно запрашивает URL по EOF, `stop` выходит из цикла.
...
- ~~Фронтенд на `/`~~ — исправлено (2026-09-16): статика отдаётся через
  `fs.Sub` (хелпер `staticFiles()`), корень отдаёт index.html.
